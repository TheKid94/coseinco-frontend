...
import MenuMen from "../../menu/MenuMen";
...
const Header = () => {
    const { openLogin, openCartP, openSearchH } = useSelector(
        (state) => state.ui
    );
    ...
    useEffect(() => {
        if (!itemsCart){
            setItemsCart(localStorage.getItem('carrito') ? JSON.parse(localStorage.getItem('carrito')) : [])
        }    
    }, [itemsCart])
        ...
        <header className="header">
            <div className="header__container grid-container s-grid-9">
                ...
                <div className="header__menu s-cols-4 lg-cols-2 flex-container">
                    ...
                    <div
                        className="header__menu-item lg-pxy-1"
                        onClick={handleOpenCart}
                    >
                        <div className="header__cart">
                            <ShoppingCart className="header__icon header__icon--cart" />
                            <span className="cart__items"> { itemsCart ? itemsCart.length : '0'  } </span>
                        </div>
                    </div>
                </div>
